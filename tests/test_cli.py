...
def test_freeze_command_no_session(server, tmpdir, monkeypatch):
    ...
    freeze_yaml = os.path.join(str(tmpdir), 'test.yaml')
    runner = CliRunner()
    result = runner.invoke(
        cli.command_freeze, "", input="\ny\n{}\n{}\ny".format(freeze_yaml, freeze_yaml)
    )
    assert os.path.isfile(freeze_yaml)
    try:
        with open(freeze_yaml, 'r') as f:
            yaml.safe_load(f)
    except:
        pytest.fail("Can't load yaml file generated by 'tmuxp freeze'")
